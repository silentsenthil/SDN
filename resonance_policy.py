...
# --------- Update the code below ------------
  ...
  def default_policy(self):
    ...
    match_portA = parallel([match(srcip=hostA) for hostA in portA_hosts]) + parallel([match(dstip=hostA) for hostA in portA_hosts])

    return if_(match_portA, self.portA_policy(), self.portB_policy())
